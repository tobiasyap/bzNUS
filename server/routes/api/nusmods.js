...
router.get("/:url", async (req, res, next) => {
  ...
  if (timetable) {
    ...
    for (const [moduleCode, codedTimes] of Object.entries(codedTimetable)) {
      modPromise = nusmods.getModule(YEAR, moduleCode).then(moduleData => {
        timetable[moduleCode] = utils.decodeTimes(
          moduleData,
          SEMESTER,
          codedTimes
        );
    ...
    Timetable.insert(url, timetable).catch(err => {
      console.error("Error inserting timetable.");
    });
  }
